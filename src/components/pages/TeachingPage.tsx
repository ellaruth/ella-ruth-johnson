    ...
    <div className="space-y-16 sm:space-y-20 pb-20 pt-6 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      {activeSermon && (
        <section className="bg-[#FDFCFB] rounded-3xl border border-[#E8E2D8] shadow-xs overflow-hidden">
          {/* Audio Player Bar */}
          <div className="bg-[#F5F2ED] border-b border-[#E8E2D8] p-4 sm:p-8 flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 sm:gap-6">
            <div className="flex items-center gap-3.5 w-full md:w-auto">
              <button
                id="sermon-audio-toggle"
                onClick={() => setIsPlayingAudio(!isPlayingAudio)}
                className="w-11 h-11 sm:w-12 sm:h-12 rounded-full bg-[#002366] hover:bg-[#001a4e] text-white flex items-center justify-center shrink-0 shadow-sm transition-transform active:scale-95 border border-[#D4AF37]/40 cursor-pointer touch-sm"
              ...
              <div className="space-y-0.5 min-w-0">
                <div className="flex items-center gap-2">
                  <span className={`inline-block w-2 h-2 rounded-full shrink-0 ${isPlayingAudio ? 'bg-emerald-500 animate-pulse' : 'bg-[#D4AF37]'}`} />
                  <span className="text-[10px] sm:text-[11px] font-semibold text-[#D4AF37] uppercase tracking-wider truncate block">
                    {isPlayingAudio ? 'Streaming' : 'Audio Preview'} • {activeSermon.duration}
                  </span>
                </div>
                <h3 className="font-serif text-base sm:text-xl font-bold text-[#002366] leading-tight truncate">
                  {activeSermon.title}
                </h3>
              </div>
            </div>
            ...
            <button
              ...
              className="w-full md:w-auto px-5 py-2.5 rounded-full bg-[#FDFCFB] hover:bg-[#EFEBE4] text-[#002366] text-xs font-medium border border-[#E8E2D8] transition-colors flex items-center justify-center gap-1.5 shrink-0"
            >
          ...
          <div className="p-4 sm:p-10 grid grid-cols-1 lg:grid-cols-12 gap-8 sm:gap-10">
